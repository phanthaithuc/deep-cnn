...
tf.flags.DEFINE_string("train_set", "data/train.csv",
...
tf.flags.DEFINE_string("log_path", "tensorboard/vdcnn",
                       "path to tensorboard folder")
tf.flags.DEFINE_string("saved_path", "trained_models",
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        np.savetxt(output_files, delimiter=",")
            ...
            for epoch in range(FLAGS.num_epochs):
                ...
                if epoch % FLAGS.test_interval == 0:
                    ...
                    confusion_matrix = np.zeros(
                        [num_classes, num_classes], np.int32)
                    num_samples = 0
                    while True:
                        try:
